...
/** SAX-type parser for reading model info from XML into InfluxDB
 *  @author Kay Kasemir
 */
...
public class XMLImport extends DefaultHandler
{
    ...
    /** Connection to InfluxDB archive */
    final private InfluxDBArchiveConfig config;
                    ...
                    throws SAXException
    {
        ...
        else if (element.equals(TAG_CHANNEL))
        {
            ...
            try
            {
                ...
                final InfluxDBSampleMode mode = config.getSampleMode(monitor, sample_value, period);
                final InfluxDBChannelConfig channel = config.addChannel(group, name, mode);
                if (is_enabling)
                {
