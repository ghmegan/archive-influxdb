...
public class ArchiveConfigApplication implements IApplication
{
    ...
    public Object start(final IApplicationContext context) throws Exception
    {
        ...
        final StringOption  influxdb_url = new StringOption(parser,
                "-influxdb_url", "jdbc:...", "InfluxDB URL", InfluxDBArchivePreferences.getURL());
        final StringOption  influxdb_user = new StringOption(parser,
                "-influxdb_user", "user", "InfluxDB User", InfluxDBArchivePreferences.getUser());
        final StringOption  influxdb_password = new StringOption(parser,
                "-influxdb_password", "password", "InfluxDB Password", InfluxDBArchivePreferences.getPassword());
        final StringOption  influxdb_schema = new StringOption(parser,
                "-influxdb_schema", "schema", "InfluxDB schema (table prefix), ending in '.'", InfluxDBArchivePreferences.getSchema());
        ...
        try
        {
            if (list.get())
            {
                final InfluxDBArchiveConfig config = new InfluxDBArchiveConfig(influxdb_url.get(), influxdb_user.get(),
                        influxdb_password.get(), influxdb_schema.get());
            ...
            if (do_export.get())
            {
                ...
                new XMLExport().export(out, influxdb_url.get(), influxdb_user.get(),
                        influxdb_password.get(), influxdb_schema.get(),
                        engine_name.get());
                if (out != System.out)
                    out.close();
            ...
            else if (do_import.get())
            {
                ...
                final XMLImport importer = new XMLImport(influxdb_url.get(), influxdb_user.get(),
                        influxdb_password.get(), influxdb_schema.get(),
